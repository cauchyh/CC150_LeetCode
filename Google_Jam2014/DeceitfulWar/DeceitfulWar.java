...
public class DeceitfulWar
{
	public static void main(String[] args)
	{
		try
		{
			...
			for (int caseNum=1; caseNum<=totalCase; caseNum++)
			{
				...
				float[] naoBlocks = new float[blockNum];
				float[] kenBlocks = new float[blockNum];
				copyBlocks(in, naoBlocks);
				copyBlocks(in, kenBlocks);
				int naomiWarScore = warScore(naoBlocks, kenBlocks);
				int naomiDeceitScore = deceitfulWar(naoBlocks, kenBlocks);
				printResult(naomiWarScore, naomiDeceitScore, caseNum);
			}
			in.close();
		}
		catch(IOException e)
		...
		// int res = warScore(naoBlocks, kenBlocks);
		// printResult(res);
	}

	private static int deceitfulWar(float[] naoBlocks, float[] kenBlocks)
	{
		int naoScore = 0;
		float[] kenBlocksClone = kenBlocks.clone();
		float[] naoBlocksClone = naoBlocks.clone();
		int naoPosition = 0;
		for (int i=naoBlocks.length-1; i>=0; i--)
		{
			for (int j=naoBlocks.length-1; j>=0; j--)
			{
				if (naoBlocksClone[i] > kenBlocksClone[j] && kenBlocksClone[j] != -1)
				{
					naoScore++;
					naoBlocksClone[i] = -1;
					kenBlocksClone[j] = -1;
		...
		return naoScore;
		// int kenFinal = kenBlocks.length - 1;
		// for (int i=0; i<naoBlocks.length; i++)
		// {
		// 	if (naoBlocks[i] > kenBlocksClone[kenFinal] && kenBlocksClone[kenFinal] != -1)
		// 	{
		// 		return naoBlocks.length - i;
		// 	}
		// 	else
		// 	{
		// 		kenBlocksClone[kenFinal] = -1;
		// 		kenFinal --;
		// 	}
		// }
		// return 0;
	...
	private static int warScore(float[] naoBlocks, float[] kenBlocks)
	{
		float[] kenBlocksClone = kenBlocks.clone();
		int naoScore = 0;
		// int kenChoice = 0;
		for (int i=0; i<naoBlocks.length; i++)
		{
			for (int j=0; j<kenBlocks.length; j++)
			{
				...
				if (naoBlocks[i] < kenBlocksClone[j] && kenBlocksClone[j] != -1)
				{
					kenBlocksClone[j] = -1; // this block is used 
					break;
				}
				if (naoBlocks[i] > kenBlocksClone[j] && j == kenBlocks.length-1)
				{
					giveSmallest(kenBlocksClone);
					naoScore++;
				}
			}
		}
	...
	private static void copyBlocks(BufferedReader in, float[] blocks)
	{
		try
		{
			...
			for (int i=0; i<temp.length(); i++)
			{
				if (temp.charAt(i) == ' ')
				{
					queue.add(Float.parseFloat(temp.substring(start, i)));
					start = i+1;
				}
			}
			queue.add(Float.parseFloat(temp.substring(start, temp.length())));
			for (int i=0; i<blocks.length; i++)
			{
				blocks[i] = queue.remove();
			}
			Arrays.sort(blocks);
			// for (int i=0; i<blocks.length; i++)
			// {
	...
	private static void giveSmallest(float[] kenBlocksClone)
	{
		for (int i=0; i<kenBlocksClone.length; i++)
		{
			if (kenBlocksClone[i] != -1)
			{
				kenBlocksClone[i] = -1;
				return;
			}
	...
	private static void printResult(int naomiWarScore, int naomiDeceitScore, int caseNum)
	{
		try
		{
			// Date date = new Date();
			...
			// Case #1: 1.0000000
			out.write("Case #" + caseNum + ": " + naomiDeceitScore + " " + naomiWarScore);
			out.newLine();
			out.close();
		}
		catch(IOException e)
		...
		// System.out.println("Case #" + caseNum + ": " + naomiDeceitScore + " " + naomiWarScore);
	}	
}
